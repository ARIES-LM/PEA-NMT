...
from data.plain import convert_data, data_length, convert_postag
...
def parseargs_train(argv):
    ...
    data.add_argument('--corpus', nargs=4, help=msg)
    msg = 'source and target vocabulary'
    data.add_argument('--vocab', nargs=4, help=msg)
    ...
    network.add_argument('--lambda', nargs=2, type=float, help="loss lambda")
    ...
    network.add_argument('--embdim', nargs=4, type=int, help=msg)
    msg = 'source, target and alignment hidden size, default 1000'
    network.add_argument('--hidden', nargs=8, type=int, help=msg)
...
def default_option():
    ...
    option["lambda"] = [1.0, 1.0]
    # model parameters
    option["embdim"] = [620, 620, 150, 150]
    option["hidden"] = [1000, 1000, 1000, 500, 500, 1200]
    option["maxpart"] = 2
    option["maxhid"] = 500
    option["deephid"] = 620
...
def override(opt1, opt2):
    ...
    if opt1["vocab"] is None:
        ...
        stag = load_vocab(opt2["vocab"][2])
        ttag = load_vocab(opt2["vocab"][3])
        stag[opt1["eos"]] = len(stag)
        ttag[opt1["eos"]] = len(ttag)
        opt1["vocabulary"].append([stag, ttag])


def print_option(option):
    isvocab = option["vocabulary"][0][1]
    itvocab = option["vocabulary"][1][1]
    stag = option["vocabulary"][2][0]
    ttag = option["vocabulary"][2][1]

    print "options"

    print "\n[new options]"
    print "tag vocab size:", [len(stag), len(ttag)]
...
def train(args):
    ...
    svocabs, tvocabs, tagvocabs = option["vocabulary"]
    svocab, isvocab = svocabs
    tvocab, itvocab = tvocabs
    unk_sym = option["unk"]
    eos_sym = option["eos"]
    stag, ttag = tagvocabs
    # istag = invert_vocab(stag)
    # ittag = invert_vocab(ttag)
    ...
    try:
        while progress.epoch < maxepoch:
            ...
            for data in stream:
                ...
                xpos, _ = convert_postag(data[2], stag, eos_sym)
                ypos, _ = convert_postag(data[3], ttag, eos_sym)

                t1 = time.time()
                # cost, norm = trainer.optimize(xdata, xmask, ydata, ymask)
                posxcost, posycost, transcost, totalcost, norm = \
                    trainer.optimize(xdata, xmask, ydata, ymask, xpos, ypos)

                trainer.update(alpha=alpha)
                t2 = time.time()

                # translation per word cost
                w_cost = transcost * ymask.shape[1] / ymask.sum()

                progress.batch_count += 1
                progress.batch_total += 1
                progress.loss_hist.append([posxcost, posycost, transcost])

                count = progress.batch_count

                if not args.pfreq or count % args.pfreq == 0:
                    print epc + 1, progress.batch_count, w_cost, norm, t2 - t1
                ...
                if count % option["sfreq"] == 0:
                    ...
                    # outputs = trainingbeamsearch([istag, ittag], model, xdata, xmask)
                    best, score = hls[0]
                    print "--", sdata
                    print "--", tdata
                    print "--", " ".join(best[:-1])
            ...
            loss_hist = progress.loss_hist
            progress.loss_hist = list()
            loss_hist = numpy.array(loss_hist)
            loss_hist = numpy.mean(loss_hist, 0).tolist()
            print "report epoch loss: posx:{}, posy:{}, trans:{}".format(*loss_hist)
...
def decode(args):
    ...
    # use the first model
    svocabs, tvocabs, _ = models[0].option["vocabulary"]
